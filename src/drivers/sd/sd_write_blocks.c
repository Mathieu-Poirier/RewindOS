 ...
 * SD spec allows up to 250 ms for programming. */
...
int sd_wait_card_ready(void)
{
        ...
        /* g_sd_info.rca is stored in SD init as RESP1 & 0xFFFF0000,
         * i.e. already positioned for addressed commands. */
        uint32_t arg = g_sd_info.rca;
...
static int sd_write_block(uint32_t addr, const uint32_t *src_words)
{
        ...
        /* RM0410 46.6.3: for writes, send CMD24 BEFORE enabling DPSM */
        ...
        uint32_t timeout = SDMMC_SW_TIMEOUT;
        while (words_left > 0u)
        {
        ...
        /* Card may keep DAT0 busy while programming internal flash after DATAEND. */
